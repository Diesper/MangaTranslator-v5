// popup.js — Manga Translator v4.0
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadRestoreEntries(chapterList) {
        ...
        const migrated = new Set(entries.map(e => `${e.chapterId}::${e.cleanUrl}`));

        // Fallback legado, só para capítulos sem nenhuma entrada nova
        ...
        if (pending.length > 0) {
            ...
            pending.forEach(chapter => {
                ...
                Object.keys(restoreMap).forEach(cleanUrl => {
                    if (migrated.has(`${chapter.id}::${cleanUrl}`)) return;
    ...
    function deleteSavedTranslationForEntry(entry) {
        ...
        (async () => {
            ...
            showSettingsStatus('Falha ao apagar a tradução salva.', '#FF9800');
        });
    }
